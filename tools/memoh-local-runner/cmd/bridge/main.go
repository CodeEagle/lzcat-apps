...
var templateDir = "/opt/memoh/templates"
...
func main() {
	...
	// Allow overriding workdir and template dir for per-bot local runner.
	if w := os.Getenv("BRIDGE_WORKDIR"); w != "" {
		defaultWorkDir = w
	}
	if td := os.Getenv("BRIDGE_TEMPLATE_DIR"); td != "" {
		templateDir = td
	}
