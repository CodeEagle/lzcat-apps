# dvc Upstream Deployment Checklist

## 已确认字段
- PROJECT_NAME: dvc
- PROJECT_SLUG: dvc
- UPSTREAM_REPO: treeverse/dvc
- UPSTREAM_URL: https://github.com/treeverse/dvc
...
## 预填环境变量
- `USER_NAME`: From compose service git-server (required=False)
- `PUBLIC_KEY_FILE`: From compose service git-server (required=False)

## 预填数据路径
- `/tmp/key` <= `/lzcapp/var/data/dvc/git-server/key` (From compose service git-server)
- `/custom-cont-init.d` <= `/lzcapp/var/data/dvc/git-server/custom-cont-init-d` (From compose service git-server)
...
## 当前服务拓扑初稿
- `git-server`
  image: `registry.lazycat.cloud/placeholder/dvc:git-server`
  binds: `/lzcapp/var/data/dvc/git-server/key:/tmp/key, /lzcapp/var/data/dvc/git-server/custom-cont-init-d:/custom-cont-init.d`
  environment: `USER_NAME=user, PUBLIC_KEY_FILE=/tmp/key`
