...
本目录由 `scripts/bootstrap_migration.py` 生成，用于把上游 `CodeEagle/copilot-api-plus-test-test` 初始化为懒猫微服迁移项目。

## 上游项目
- Upstream Repo: CodeEagle/copilot-api-plus-test-test
...
4. 初稿补全后执行 `./scripts/local_build.sh copilot-api-plus-test-test --check-only`，再进入实际构建与验收。
